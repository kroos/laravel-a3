...
<div class="card">
	...
	<h6>Choose super super shue according to your character type</h6>

	<form method="POST" action="{{ route('equipsupersupershue.store') }}" id="form" accept-charset="UTF-8" enctype="multipart/form-data">
		@csrf
			...
			<div class="form-group row {{ $errors->has('m_body') ? ' has-error' : '' }}">
				{{ Form::label( 'hero', 'Super Super Shue : ', ['class' => 'col-4 col-form-label text-right'] ) }}
				<div class="col-6">
					{{ Form::select('m_body', config('a3.supersupershue'), @$value, ['class' => 'form-control'.($errors->has('m_body') ? ' is-invalid' : NULL), 'id' => 'hero', 'placeholder' => 'Please choose']) }}
