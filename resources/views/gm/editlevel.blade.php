...
<div class="card">
	...
	<h6>Please take note that only level 100 n 165 is available. Will update more if got time.... ;)</h6>

	<form method="POST" action="{{ route('editlevel.store') }}" id="form" accept-charset="UTF-8" enctype="multipart/form-data">
		@csrf
			...
			<div class="form-group row {{ $errors->has('m_body') ? ' has-error' : '' }}">
				{{ Form::label( 'hero', 'Level : ', ['class' => 'col-4 col-form-label text-right'] ) }}
				<div class="col-6">
					{{ Form::select('m_body', config('a3.editlevel'), @$value, ['class' => 'form-control'.($errors->has('m_body') ? ' is-invalid' : NULL), 'id' => 'hero', 'placeholder' => 'Please choose']) }}
