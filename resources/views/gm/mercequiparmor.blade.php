...
<div class="card">
	<div class="card-header">
		<h1>Mercenary Equip Armor</h1>
	</div>
	<div class="card-body">
	@include('layouts.info')
	@include('layouts.errorform')

	<h6>All this equipment is complete with Redyan Necklace and Iarrtek</h6>

	<form method="POST" action="{{ route('mercequiparmor.store') }}" id="form" accept-charset="UTF-8" enctype="multipart/form-data">
		@csrf
			...
			<div class="form-group row {{ $errors->has('hsbody') ? ' has-error' : '' }}">
				{{ Form::label( 'armor', 'Armor : ', ['class' => 'col-4 col-form-label text-right'] ) }}
				<div class="col-4">
					{{ Form::select('hsbody', config('a3.armor'), @$value, ['class' => 'form-control'.($errors->has('hsbody') ? ' is-invalid' : NULL), 'id' => 'armor', 'placeholder' => 'Please choose']) }}
					@if ($errors->has('hsbody'))
					<span class="invalid-feedback" role="alert">
						<strong>{{ $errors->first('hsbody') }}</strong>
					</span>
					@endif
				</div>
			</div>
