...
<div class="card">
	...
	{!! Form::model($buy_all_skill, ['route' => ['bas.update', $buy_all_skill->c_id], 'method' => 'PATCH', 'id' => 'form', 'files' => true]) !!}


		<h6 class="mbr-section-subtitle align-center mbr-light pb-3 mbr-fonts-style display-5">Please select your character<br />Please Logout Before Submit This Form</h6>
<h6>Condition : your heroes have {{ config('a3.wzbuyskill') }}Wz and its level is greater or equal than {{ config('a3.levelbuyskill') }}</h6>
<h6>Please note, once you reach rebirth level 1, level cap will be void.</h6>



		@if($buy_all_skill->hasmanycharac()->where('c_status', 'A')->count() > 0)

			<fieldset class="form-group {{ $errors->has('c_id') ? ' has-error' : '' }}">
				<div class="row">
					<legend class="col-form-label col-4 pt-0 text-right">Hero :</legend>
					<div class="col-6">
					@foreach($buy_all_skill->hasmanycharac()->where('c_status', 'A')->get() as $char)
